    ...
    public class FillFormService : Base.BaseServices<FillForm>, IFillFormService
    {
        ...
        public async Task<List<FillformDto>> GetFillformDtos(int pageindex, int pagesize, int UserId)
        {
            ...
            foreach (var item in data)
            {
                ...
                models.Mounth = item.Mounth;
                models.Year = item.Year;             
                models.Tables = _Mapper.Map<List<ProjectMinDto>>(JsonConvert.DeserializeObject<List<ProjectDto>>(item.TableData));
                if (item.TableFour != string.Empty)
                {
                    models.TablesFour = _Mapper.Map<List<ProjectMinFourDto>>(JsonConvert.DeserializeObject<List<ProjectFourDto>>(item.TableFour));
                }
                else {

                    models.TablesFour = new List<ProjectMinFourDto>();
                }
